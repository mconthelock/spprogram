...
export async function tablePartOption(data = []) {
	...
	opt.columns = [
		...
		},
		{
			data: "INQD_QTY",
			title: "Qty.",
			className: "px-[3px]! text-right bg-primary/20 INQD_QTY",
			footer: "Total",
			render: function (data, type) {
				// if (type === "display") {
				// 	data = data == null ? "" : data;
				// 	return `<div class="px-2"><input type="text" class="w-full min-w-13.75 outline-0 text-right input-number inqty" value="${data}"/></div>`;
				// }
				if (type === "display") {
	...
	opt.footerCallback = function () {
		...
		api.column(1).footer().className = "sticky-column";
		api.column(2).footer().className = "sticky-column";
		api.column(3).footer().className = "sticky-column";
		api.column(4).footer().className = "sticky-column";
		api.column(5).footer().className = "sticky-column";
		api.column(11).footer().innerHTML = "XX";
