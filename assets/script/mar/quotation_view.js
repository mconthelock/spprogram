...
import { setupCard } from "../inquiry/index.js";
import {
	getInquiryID,
	getInquiryHistory,
	getInquiryFile,
...
$(document).ready(async () => {
	try {
		...
		const inquiry = await getInquiryID($("#inquiry-id").val());
		if (inquiry.length == 0) throw new Error("Inquiry do not found");

		$("#inquiry-title").html(inquiry.INQ_NO);
		inquiry.QUO_DATE = dayjs(inquiry.quotation.QUO_DATE).format(
			"YYYY-MM-DD",
		);
		inquiry.QUO_VALIDITY = dayjs(inquiry.quotation.QUO_VALIDITY).format(
			"YYYY-MM-DD",
		);
		inquiry.QUO_NOTE =
		...
		const tableContainer = await setupTableDetail(
			details,
			inquiry.INQ_TYPE,
		);
